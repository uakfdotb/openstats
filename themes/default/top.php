...
<?php if (isset($_GET["sort"]) AND $_GET["sort"] == "player_name" ) $sel = 'selected="selected"'; else $sel = ''; ?>
    ...
    <table>
     <tr> 
	   ...
	   <th width="160" class="alignleft"><?=$lang["player"]?></th>
	   <th width="80" class="alignleft"><?=$lang["score"]?></th>
	   <th width="80" class="alignleft"><?=$lang["games"]?></th>
	   <th width="90" class="alignleft"><?=$lang["wld"]?></th>
	   <th width="90" class="alignleft"><?=$lang["wl_percent"]?></th>
	   <th width="120" class="alignleft"><?=$lang["kda"]?></th>
	   <th width="120" class="alignleft"><?=$lang["cdn"]?></th>
	   <th width="120" class="alignleft"><?=$lang["tr"]?></th>
	  </tr>
<?php 
foreach ($TopData as $Data) {
  ...
  <tr class="row">
    ...
    <td width="160" class="alignleft">
	<?php if (isset($Data["letter"]) AND !empty($Data["letter"]) ) { ?>
	<img class="imgvalign" width="21" height="15" src="<?=$website?>img/flags/<?=$Data["letter"]?>.gif" alt="" />
	<?php } ?>
	<a href="<?=$website?>?u=<?=$Data["id"]?>"><?=$Data["player"]?></a>
	</td>
	<td width="80" class="alignleft"><?=$Data["score"]?></td>
    <td width="80" class="alignleft"><?=$Data["games"]?></td>
	<td width="90" class="alignleft">
	  <span class="won"><?=$Data["wins"]?></span>/
	  <span class="lost"><?=$Data["losses"]?></span>/
	  <span class="draw"><?=$Data["draw"]?></span>
	  </td>
	<td width="90" class="alignleft"><?=$Data["winslosses"]?>%</td>
